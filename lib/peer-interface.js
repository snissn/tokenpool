    ...
    if (!cluster.isMaster) { return; }
     for(i in solution_txes)
     {
       try{
         ...
         //console.log('TRANSACTINO DATA',transactionData);
...
//}
  ...
  async getFeeDynamicByGasPrice(){
    ...
    var fetch_api = await fetch("https://api.etherscan.io/api?module=gastracker&action=gasoracle&apikey=79YJDRSD7DS92J993F5QVMDPVGBS41RGPM")
    var result = await fetch_api.json()
    var gas =  result['result']['FastGasPrice']* 1000000000 * 1//.25
    gas = Math.min(gas,179000000000)
    var eth_used =  gas * 93026 /1E18; //100,000 is appxtly the amount of gas used per block, 1e18 is the ratio of gWei to eth
    ...
    var percent = 8.0 + tokens_needed_fee/50*100
    return percent/100
  },
